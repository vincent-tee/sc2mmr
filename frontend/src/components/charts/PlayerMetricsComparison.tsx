...
interface MetricsData {
  [playerId: number]: PlayerMetrics;
}

interface PlayerMetricsComparisonProps {
...
const PlayerMetricsComparison: React.FC<PlayerMetricsComparisonProps> = ({
  players,
  metricsData,
}) => {
  ...
  // Color by team
  const getPlayerColor = (teamNumber: number): string => {
    return teamNumber === 1 ? '#00D4FF' : '#FFB300';
  };

  // Prepare comparison data
  const economicData: ChartDataItem[] = players.map((player) => ({
  ...
  const combatData: ChartDataItem[] = players.map((player) => ({
    name: player.player_name.substring(0, 10),
    fullName: player.player_name,
  ...
  const overallData: ChartDataItem[] = players.map((player) => ({
    name: player.player_name.substring(0, 10),
    fullName: player.player_name,

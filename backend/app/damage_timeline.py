...
class DamageTimeline:
    ...
    def get_window_damage(self, start: int, end: int) -> int:
        ...
        return sum(dmg for sec, dmg in self.damage_events.items() if start <= sec < end)
    ...
    def get_first_damage_second(self) -> Optional[int]:
        ...
        if not self.damage_events:
            return None
        return int(min(self.damage_events.keys()))
    ...
    def get_peak_damage_second(self) -> Optional[Tuple[int, int]]:
        ...
        if not self.damage_events:
            return None
        second = int(
            max(self.damage_events.keys(), key=lambda k: self.damage_events[k])
        )
        return (second, int(self.damage_events[second]))
    ...
    def get_damage_spikes(self, threshold: Optional[int] = None) -> List[DamageSpike]:
        ...
        if threshold is None:
            ...
            threshold = int(avg_damage * 3)

        spikes = [
            DamageSpike(second=int(sec), damage=int(dmg))
    ...
    def detect_timing_attacks(
        ...
        if spike_threshold is None:
            avg_damage = self.get_average_damage_per_active_second()
            spike_threshold = int(avg_damage * 2)
            ...
            if damage >= spike_threshold:
                if current_attack is None:
                    ...
                    current_attack = {
                        "start": int(second),
                        "end": int(second),
                        "peak_second": int(second),
                        "peak_damage": damage,
                        "total_damage": damage,
                        "last_second": int(second),
                    }
                elif int(second) - current_attack["last_second"] <= max_gap:
                    # Continue current attack
                    current_attack["end"] = int(second)
                    current_attack["total_damage"] += damage
                    current_attack["last_second"] = int(second)

                    if damage > current_attack["peak_damage"]:
                        current_attack["peak_second"] = int(second)
...
class DamageTimelineExtractor:
    ...
    def extract_from_replay(
        ...
        damage_by_second: Dict[int, int] = defaultdict(int)
    ...
    def calculate_coordination_score(
        timeline1: DamageTimeline, timeline2: DamageTimeline, window_seconds: int = 10
    ) -> float:
        ...
        coordination = (
            (coordinated / total_attacks) * 100 if total_attacks > 0 else 50.0
        )

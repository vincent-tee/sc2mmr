...
class MatchCommentaryGenerator:
    ...
    def generate_match_summary(
        ...
        commentary = {
            'match_id': match_id,
            'overview': MatchCommentaryGenerator._generate_match_overview(match, match_players),
            'key_moments': MatchCommentaryGenerator._generate_key_moments(match, players_with_metrics),
            'player_performances': MatchCommentaryGenerator._generate_player_performances(players_with_metrics),
            'mvp_analysis': MatchCommentaryGenerator._generate_mvp_analysis(players_with_metrics),
            'team_analysis': MatchCommentaryGenerator._generate_team_analysis(players_with_metrics),
            'match_summary': MatchCommentaryGenerator._generate_final_summary(match, players_with_metrics)
        }
    ...
    def _generate_match_overview(match: Match, match_players: List[MatchPlayer]) -> str:
        ...
        if match.duration_seconds < 300:
            ...
            overview += "This was an extended battle with both teams fighting for position. "
    ...
    @staticmethod
    def _generate_key_moments(match: Match, players_with_metrics: List[Dict]) -> List[str]:
        ...
        for p in players_with_metrics:
            if p['metrics'] and p['metrics'].first_expansion_timing:
                early_expands.append((p['player'].name, p['metrics'].first_expansion_timing))
        ...
        early_damage = []
        for p in players_with_metrics:
            if p['metrics'] and hasattr(p['metrics'], 'first_damage_timing') and p['metrics'].first_damage_timing:
                if p['metrics'].first_damage_timing < 180:  # Under 3 minutes
                    early_damage.append((p['player'].name, p['metrics'].first_damage_timing))
        ...
        top_impacts = sorted(
            [(p['player'].name, p['metrics'].overall_impact) for p in players_with_metrics if p['metrics']],
            key=lambda x: x[1],
            reverse=True
        )
        ...
        # Economic powerhouse
        top_economy = sorted(
            [(p['player'].name, p['metrics'].total_resources_collected) for p in players_with_metrics if p['metrics']],
            key=lambda x: x[1],
            reverse=True
        )
        ...
        # Combat excellence
        top_damage = sorted(
            [(p['player'].name, p['metrics'].damage_dealt, p['metrics'].damage_ratio) for p in players_with_metrics if p['metrics']],
            key=lambda x: x[1],
            reverse=True
        )
    ...
    @staticmethod
    def _generate_player_performances(players_with_metrics: List[Dict]) -> Dict[str, str]:
        ...
        for p in players_with_metrics:
            ...
            mmr_change = f"+{int(abs(mp.mmr_change))}" if mp.mmr_change > 0 else f"-{int(abs(mp.mmr_change))}"
            ...
            if metrics.damage_ratio > 2.0:
                highlights.append(f"exceptional {metrics.damage_ratio:.1f}:1 damage efficiency")

            if metrics.spending_efficiency > 0.9:
                highlights.append(f"excellent {metrics.spending_efficiency*100:.0f}% spending efficiency")

            if metrics.units_killed > metrics.units_lost * 2:
                highlights.append(f"dominant {metrics.units_killed} kills to {metrics.units_lost} losses")
        ...
        if winners:
            ...
            if mvp_metrics:
                ...
                if mvp_metrics.combat_score >= 70:
                    reasons.append(f"exceptional combat performance ({mvp_metrics.combat_score:.1f}/100)")

                if mvp_metrics.economic_score >= 70:
                    reasons.append(f"strong economic foundation ({mvp_metrics.economic_score:.1f}/100)")

                if mvp_metrics.damage_ratio > 2.5:
                    reasons.append(f"outstanding {mvp_metrics.damage_ratio:.1f}:1 damage efficiency")
                ...
                analysis['reasoning'] = f"{mvp_name} made the biggest contribution to their team's victory."
    ...
    def _generate_team_analysis(players_with_metrics: List[Dict]) -> Dict:
        ...
        def analyze_team(team, team_num):
            ...
            if avg_efficiency > 70:
                ...
                analysis += "The team struggled with resource efficiency and decision-making. "

            return analysis

        return {
            'team_1': analyze_team(team_1, 1),
            'team_2': analyze_team(team_2, 2)
        }
    ...
    def _generate_final_summary(match: Match, players_with_metrics: List[Dict]) -> str:
        ...
        team_1_impact = sum(p['metrics'].overall_impact for p in team_1 if p['metrics']) / len(team_1) if team_1 else 0
        team_2_impact = sum(p['metrics'].overall_impact for p in team_2 if p['metrics']) / len(team_2) if team_2 else 0
        ...
        if impact_diff < 10:
            summary += "Both teams performed admirably with nearly matched impact scores, "
        ...
        all_impacts = [(p['player'].name, p['metrics'].overall_impact) for p in players_with_metrics if p['metrics']]
        if all_impacts:
            top_player = max(all_impacts, key=lambda x: x[1])
            summary += f"{top_player[0]} stood out as the top performer across both teams."

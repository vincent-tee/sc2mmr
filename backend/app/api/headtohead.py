...
def get_biggest_rivalries(limit: int = 20, db: Session = Depends(get_db)):
    ...
    """
    rivalries = db.query(PlayerRivalry)\
    ...
    results = []
    for r in rivalries:
        results.append(RivalryResponse(
...
def get_player_rivals(player_id: int, limit: int = 10, db: Session = Depends(get_db)):
    ...
    rivalries = db.query(PlayerRivalry).filter(
        or_(PlayerRivalry.player1_id == player_id, PlayerRivalry.player2_id == player_id)
    ).order_by(desc(PlayerRivalry.rivalry_score)).limit(limit).all()
...
def get_head_to_head(player1_id: int, player2_id: int, db: Session = Depends(get_db)):
    ...
    if rivalry:
        ...
        stats = HeadToHeadStats(
            ...
            win_rate_player1=req_p1_wins / rivalry.games_against if rivalry.games_against > 0 else 0.0,
    ...
    return HeadToHeadResponse(
        player1=PlayerSummary(
            id=req_p1.id, name=req_p1.name, mmr=req_p1.mmr,
            wins=req_p1.wins, favorite_race=req_p1.favorite_race
        ),
        player2=PlayerSummary(
            id=req_p2.id, name=req_p2.name, mmr=req_p2.mmr, 
            wins=req_p2.wins, favorite_race=req_p2.favorite_race
        ),
        head_to_head=stats,
